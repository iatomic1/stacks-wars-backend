    ...
    try {
      ...
      if (word.length < (room.minWordLength || 4)) {
        socket.emit("wordRejected", {
          reason: `Word must be at least ${room.minWordLength || 4} letters long`,
        });
        return;
      }
      ...
      const randomLetter =
        room.currentRule.rule.match(/'([a-z])'/i)?.[1] || getRandomLetter();
      ...
      io.to(roomId).emit("wordSubmitted", {
        ...
        currentPlayer:
          updatedRoom.players.find((p) => p.isCurrentPlayer)?.username || "",
      });
    } catch (error) {
      console.error(`Error submitting word: ${error}`);
      socket.emit("error", { message: "Failed to process word submission" });
    }
  };
  ...
  try {
    ...
    for (const roomId of socketRooms) {
      ...
      if (playerIndex !== -1) {
        ...
        if (room.players[playerIndex].isCurrentPlayer && room.currentRule) {
          await moveToNextPlayer(roomId, io);
        }
